...
- **미매칭 행의 범위 처리는 범위 종류에 따라 다름** (`MatchScopeInfo.ExcludesUnmatched`):
  - **선택 항목 / Clipping 영역**(양성 포함 판정): 미매칭 행은 모델 노드가 없어 선택/단면 안에 *있을 수가 없으므로* 제외. → 이 범위에선 `미매칭` 카운트가 0으로 줄고, "정확히 매칭된 것만 선택 → 미매칭 안 뜸"이 성립.
  - **숨김 제외**(음성 배제 판정): 미매칭 행은 숨김 대상도 아니므로 그대로 표시. 현황 라벨에 `(… 기준, 미매칭은 전체)` 병기.
  - **전체 모델**: 전부 표시.
  - (Spool/Hydrotest/Equipment/EIT는 미매칭이 노드 없음 → all-or-nothing. Cable은 미매칭 노드도 box가 있어 위 규칙과 무관하게 개별 판정됨.)
