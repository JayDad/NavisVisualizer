...
- **클립보드 복사 → `선택항목 클립보드 복사`** (전 탭, 2026-07 사용자 요청): 동작은 기존과 동일
  (선택 없으면 표시 중인 전체 행 — 툴팁으로 안내). 복사 피드백 "클립보드에 N행 복사됨"은 종전엔
  `_lblStats`를 덮어써 단계 현황이 사라졌는데, 이제 통계 행 우측 코너(미매칭 오른쪽)의 전용 라벨
  `_lblCopied`에 표시 후 4초 뒤 자동 소거 (Spool/Hydro/Equip/EIT/Cable 5탭. Sub-system은 레이아웃이
  달라 버튼명만 통일).
...
#### 19-부록. Ruya(Q558) NWD 파일 규약 비호환 — 2026-07 초기 분석 기록 (위 매핑 구현으로 대체됨, 참고용)
...
### 21. 일괄 갱신(배치) — "아침에 딸각" (구현됨 2026-09 — Windows 검증 대기)
