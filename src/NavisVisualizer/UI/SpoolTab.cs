...
namespace NavisVisualizer.UI
{
    public class SpoolTab : UserControl, IOverviewSource
    {
        ...
        private void BuildIndex()
        {
            ...
            ScopeGate.EnsureMapped(this, doc, NwdScope.Spool);
        ...
        public OverviewStatus GetOverviewStatus()
        {
            ...
            return new OverviewStatus
            {
                ...
                ScopeUnmapped = _main.SpoolTagSearcher.LastScopeUnmapped,
            };
        }
        ...
        private void FilterList()
        {
            ...
            // 검색 범위: 전체(-1) = SearchKey, 특정 열 = 그 열 값(SortKeySelector)만.
            if (!string.IsNullOrEmpty(keyword))
            {
                ...
                {
                    var sel = SortKeySelector(scopeCol);   // 행마다 델리게이트 생성 방지
                    filtered = filtered.Where(s => (sel(s) ?? "").ToUpperInvariant().Contains(keyword));
                }
            }
